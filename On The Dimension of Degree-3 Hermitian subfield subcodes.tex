...
\end{align*}
In other words, $B=B(\beta_1)$ is a well-defined element of the general unitary group $GU(3,q)$. The set $\mathcal{B}=\{B(\beta_1) \mid \beta_1 \in \mathbb{F}_{q^6}, \; \beta_1^{q^3+1}=1\}$ is a cyclic group of unitary transformations, its order is $|\mathcal{B}|=q^3+1$. 
...
\[3|G(P)| = \begin{cases}
\frac{q(q-1)}{2} & \text{if $q\equiv 0,1 \pmod3$,} \\
\frac{q(q-1)}{2}-1 & \text{if $q\equiv 2 \pmod3$.} \\
\end{cases}\]
\end{lemma}
\begin{proof}
...
\[U_{t,i}=\frac{\ell_i\ell_{i+1}^q}{(\ell_1\ell_2\ell_3)^((q+1)/3)},\]
...
%\[W_{s,\alpha} = \alpha U_{s,1} + \alpha^{q^2} U_{s,2} + \alpha^{q^4} U_{s,3}, \qquad \alpha \in \mathbb{F}_{q^6}.\]
