...
\item For $\mathcal{W}_t$ and $\mathcal{W}_s$ are isomorphic $\mathbb{F}_{q^2}\mathcal{B}^*$-modules if and only if $s\equiv t \pmod{q^2-q+1}$, $s\equiv (q-1)t \pmod{q^2-q+1}$, or $s\equiv -qt \pmod{q^2-q+1}$. 
\end{enumerate}
\end{proposition}
\begin{proof}
content...
...
The code $C_{\mathcal{L}}(D,sP)$ forms an $[n,k]$ AG code, where $k\geq 3s-\g+1$, achieving equality when $\lfloor\frac{ 2 \g-2}{3}\rfloor <s<n$. Additionally, the designed minimum distance is $d=q^3-3s$.

In addition to $C_{\mathcal{L}}(D,sP)$, we can define a complementary degree $3$ place functional Hermitian code associated with $sP$, denoted by $C_{\mathcal{L}}(D+P_{\infty},sP)$. This code is constructed as follows:
...
The dual code $C_{\Omega}(D,sP)$ of $C_{\mathcal{L}}(D,sP)$ is referred to as the differential code. It constitutes an $[n, \ell(sP -D)-\ell(sP) +\deg D, d^{\perp}]$ code, where $d^{\perp} \leq \deg(sP) - (2\g -2 )$, with $\deg(sP) - (2\g -2 )$ is its designed distance.
...
Moreover, the dual code $C_{\Omega}(D+P_{\infty},sP)$ of $C_{\mathcal{L}}(D+P_{\infty},sP)$
