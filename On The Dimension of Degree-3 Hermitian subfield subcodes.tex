...
%	a $[n, k \geq \ell(G)]_{\mathbb{F}_{q^2}}$ code. If $ \deg (G) < n $, its dimension is $\ell(G)$ and its minimum distance $d$ is bounded by the \emph{designed distance} $d^*=n-\deg (G)$.
	...
	\begin{theorem}\cite[Theorem~2.2.2]{stichtenoth2009algebraic}
		...
		$$
		k=\ell(G)-\ell(G-D) \quad \text { and } \quad d \geq n-\operatorname{deg} G .
		$$
	\end{theorem}
...
Let $T_{\mathbb{F}_{q^m} / \mathbb{F}_q}$ denote the trace function from $\mathbb{F}_{q^m}$ down to $\mathbb{F}_q$, expressed as
\[
T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(x) = x + x^q + x^{q^2} + \ldots + x^{q^{m-1}}.
\]
For any vector $c = (c_1, c_2, \ldots, c_n) \in \mathbb{F}_q^n$, we define
\[
T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c) = \left( T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c_1), T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c_2), \ldots, T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(c_n) \right).
\]
Furthermore, for a linear code $C$ of length $n$ and dimension $k$ over $\mathbb{F}_{q^m}$, the code $T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(C)$ is a linear code of length $n$ and dimension $k_1$ over $\mathbb{F}_q$.
...
\begin{theorem}[\cite{Del75}]\label{delsarte}
	...
	\[
	(C|_{\mathbb{F}_q})^{\perp} = T_{\mathbb{F}_{q^m} / \mathbb{F}_q}(C^{\perp}).
	\]
\end{theorem}
...
where $D=Q_1+\ldots+Q_n$ (with pairwise distinct places $Q_1, \ldots, Q_n$ of degree one), and $G=sP$ where $P$ is a degree $r$ pace on $\mathscr{H}_q$ with $\operatorname{supp} D \cap \operatorname{supp} G=\emptyset$ and $\operatorname{deg} G<n$. Suppose that $G_1$ is a divisor of $\mathbb{F}_{q^2}(\mathscr{H}_q)$ satisfying
...
\begin{equation}\label{esti1}
	\operatorname{dim} \operatorname{T_{\mathbb{F}_{q^2} / \mathbb{F}_q}}\left(\calC_{\mathcal{L}}\right) \leq \begin{cases}m\left(\ell(G)-\ell\left(G_1\right)\right)+1 & \text { if } G_1 \geq 0, \\ m\left(\ell(G)-\ell\left(G_1\right)\right) & \text { if } G_1 \ngtr 0,\end{cases}
\end{equation}
and
\begin{equation}\label{esti2}
	\left.\operatorname{dim} C_{\Omega}\right|_{\mathbb{F}_q} \geq \begin{cases}n-1-m\left(\ell(G)-\ell\left(G_1\right)\right) & \text { if } G_1 \geq 0, \\ n-m\left(\ell(G)-\ell\left(G_1\right)\right) & \text { if } G_1 \ngtr 0 .\end{cases}
\end{equation}
...
The main result of this paper deals with an $\mathbb{F}_{q^2}$-rational divisor $G=sP$ where $P$ is a degree-3 place in $\mathbb{F}_{q^2}(\mathscr{H}_q)$ and s is a positive integer. As stated above, in the extended constant field of $\mathbb{F}_{q^2}(\mathscr{H}_q)$ with degree $3$, let $P_1, P_2, P_3$ be the extensions of $P$. These points are degree-one places in $\mathbb{F}_{q^6}(\mathscr{H}_q)$, and, by appropriately labeling the indices, $P_{j+1} = \text{Fr}(P_j)$, where $\text{Fr}$ is the $q^2$-th power Frobenius map and indices are considered modulo $3$. Additionally, $P$ can be identified with the $\mathbb{F}_{q^2}$-rational divisor $P_1+P_2+P_3$ in $\mathbb{F}_{q^6}(\mathscr{H}_q)$. The Riemann-Roch space associated with the divisor $sP$ \cite{korchmaros2013hermitian} is defined as:
	...
	\textcolor{red}{some facts on the Hermitian codes and their subfield subcodes}
	\begin{proposition}
...
\begin{conjecture} 
	For a prime power $q \geq 3$, let  $\calC_{\mathcal{L}}=\calC_{\mathcal{L}}( D, (q-1)P)$ be the Hermitian code, where $P$ is a degree 3 place. Let $Tr(\calC_{\mathcal{L}})$ denotes the trace code $T_{\mathbb{F}_{q^2} / \mathbb{F}_q}(\calC_{\mathcal{L}})$. We conjecture that:
...
\[ \operatorname{dim} C_{q}(s)= \begin{cases} 1 & \text { for } 0 \leq s<2\mathfrak{g} \\ 7 & \text { for } s=2\mathfrak{g}\end{cases}\]
