...
\begin{proposition}
	Consider a positive divisor $G_1$. The trace map $Tr = \trace_{\mathbb{F}_{q^2} / \mathbb{F}_q}$ satisfies the following properties:
	\begin{enumerate}
		...
		\item Similarly, for any codeword $c \in C_{\mathcal{L}}(D,G_1)$, its trace resides in $C_{\mathcal{L}}(D,qG_1)$, which implies $Tr(C_{\mathcal{L}}(D,G_1)) \subseteq C_{\mathcal{L}}(D,qG_1)$.
	\end{enumerate}
\end{proposition}

\begin{corollary}\label{corol:6}
...
satisfies
		\[\dim_{\mathbb{F}_q} \ker(Tr) = 1.\]
	
	
	Therefore, the trace map serves as an embedding, and the dimension of $C_{\mathcal{L}}(D,qG_1)|_{\mathbb{F}_q}$ is at least one less than the dimension of $C_{\mathcal{L}}(D,G_1)$.
\end{corollary}
In our investigation, we conducted a series of experiments to compute the true dimension of the subfield subcodes $ C_{q}(s) $ for $ q \leq 16 $ and $0 \leq s \leq n$. alongside, while exploring the dimension of the Hermitian code $ C_{\mathcal{L}}(D, G) $ and its trace code, we observed an unusual behaviour in the dimension when $s = q - 1 $. This observation with Corollary \ref{corol:6} motivates the formulation of the result :

\begin{theorem}
	For a prime power $q \geq 3$, let  $\calC_{\mathcal{L}}=\calC_{\mathcal{L}}( D, (q-1)P)$ be the Hermitian code associated with a degree $3$ place $P$. Let $Tr(\calC_{\mathcal{L}})$ denotes the trace code $\trace_{\mathbb{F}_{q^2} / \mathbb{F}_q}(\calC_{\mathcal{L}})$. Then
	\[\dim Tr(\calC_{\mathcal{L}}) \geq 7 .\]
\end{theorem}



Experimental results indicate that for $0 \leq s < 2\mathfrak{g}$, the dimension of $C_{\mathcal{L}}(D,sP)_{\mid \mathbb{F}_q}$ is $1$. Additionally, in the Proposition \ref{prop:sub} presented earlier demonstrates this result for $ s =\frac{q(q-1)}{3}=\frac{2}{3} \g$.



\begin{theorem}
	For a prime power $q \geq 3$, let $C_q (s)=C_{\mathcal{L}}( D,G)_{|_{\mathbb{F}_q}}$ denote the subfield subcode of the degree-3 place one-point Hermitian code. Then 
	\[ \dim C_{q}(s)= \begin{cases} \geq 1 & \text { for } 0 \leq s<2\mathfrak{g} \\ 7 & \text { for } s=2\mathfrak{g} \text{ and } q>2\\ 10 & \text{ for } s=2\g +1 \text{ and } q>3\end{cases}\]
