...
\begin{proposition}\label{coroll}
...
\begin{proof}
	Let $f$ being a function in $\mathscr{L}(G_1)$ such that $f(Q_i) \in \mathbb{F}_q$ for $i=1,\cdots,n$. Then $f^q -f \in \mathscr{L}(G)$ (since $\mathscr{L}(G_1)^q \subseteq \mathscr{L}(G)$), hence $f^q - f \in  \mathscr{L}(G-D)$ where
...
\[C_\Omega(D,G) = C_\mathcal{L}(D-G+(dt)-(t)),\]
...
\begin{proposition}
\[C_\Omega(D,sP) = C_\mathcal{L}(D,(q^3+q^2-q-2)Q_{\infty}-sP). \qed\]
\end{proposition}
...
\[C_\Omega(\tilde{D},G) = \boldsymbol{\mu} \star C_\mathcal{L}(\tilde{D},\frac{(q^2-1)(q+1)}{3}P-G).\]
\end{theorem}
\begin{proof}
